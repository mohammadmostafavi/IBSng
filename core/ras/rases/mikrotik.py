...
class MikrotikRas(GeneralUpdateRas):
    ...
    def __getInOuts(self):
        ...
        for oid in snmp_in_bytes:
            ...
            try:
                inout_bytes[port] = {"in_bytes": snmp_in_bytes[oid],
                                     "out_bytes": snmp_out_bytes["%s.%s" % (out_bytes_oid, port)]}
    ...
    def handleRadAcctPacket(self, ras_msg):
        ...
        if status_type == "Start":
            ras_msg.setInAttrs(
                {"User-Name": "username", "Framed-IP-Address": "remote_ip", "Acct-Session-Id": "session_id"})
            ...
            self.onlines[ras_msg["port"]] = {"username": ras_msg["username"], "in_bytes": 0, "out_bytes": 0,
                                             "in_rate": 0, "out_rate": 0, "last_update": time.time()}
            ...
            try:
                self.inouts[ras_msg["port"]]["in_bytes"], self.inouts[ras_msg["port"]]["out_bytes"] = ras_msg[
                    "in_bytes"], ras_msg["out_bytes"]
            except KeyError:
            ...
            if ras_msg["port"] in self.onlines:
                ...
                self.onlines[ras_msg["port"]]["in_rate"] = (in_bytes - self.onlines[ras_msg["port"]][
                    "in_bytes"]) / duration
                self.onlines[ras_msg["port"]]["out_rate"] = (out_bytes - self.onlines[ras_msg["port"]][
                    "out_bytes"]) / duration
                ...
                self.toLog("Update accounting called for %s,%s while he's on my online list" % (
                ras_msg.getRequestAttr("User-Name")[0], ras_msg["port"]))
        else:
