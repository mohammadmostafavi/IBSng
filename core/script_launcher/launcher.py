...
class ScriptLauncher:
    ...
    def __launchWithScriptWrapper(self, method, script, args, timeout, shell_pipes=""):
        ...
        # cmd = "%sscript_wrapper/script_wrapper.py -t %s -- '%s' %s %s"%(defs.IBS_ADDONS,
        #                                                                         timeout,
        #                                                                         real_script,
        #                                                                         self.__prepareScriptArgs(script_args + args),
        #                                                                         shell_pipes)
        cmd = f"python {defs.IBS_ADDONS}script_wrapper/script_wrapper.py -t {timeout} -- '{real_script}' {self.__prepareScriptArgs(script_args + args)} {shell_pipes}"
        if self.DEBUG:
